...
1.  เข้าไปที่โฟลเดอร์ `app-vite/`
2.  รันคำสั่ง `npm install`
3.  รันคำสั่ง `npm run dev:electron` เพื่อเริ่มระบบทั้ง Frontend และ Electron App พร้อมกัน

### 5.2 การ Build เพื่อพกพา/ใช้งานจริง (Portable/Production)
1.  **Build Application**:
    - เข้าไปที่โฟลเดอร์ `app-vite/`
    - รันคำสั่ง `npm run build:win` (สำหรับ Windows) หรือ `npm run build:mac` (สำหรับ macOS)
    - ระบบจะสร้างไฟล์ผลลัพธ์ไว้ที่โฟลเดอร์ `dist-electron-vite/`

2.  **Package ลง USB**:
    - ใช้สคริปต์อัตโนมัติที่เตรียมไว้เพื่อจัดเตรียมไฟล์สำหรับ USB
    - รันคำสั่งจาก root ของโปรเจกต์: `./scripts/package-fresh-usb.sh`
    - สคริปต์จะทำการ:
        - คัดลอกตัวโปรแกรมที่ Build แล้ว
        - สร้าง Launcher scripts (`START_WINDOWS.bat`, `START_MAC.command`)
        - เตรียมโฟลเดอร์ `data` และ `uploads`
        - บีบอัดเป็นไฟล์ zip พร้อมนำไปแตกไฟล์ลง USB ได้ทันที

## 7. กระบวนการอัปเดตและเผยแพร่ (Release Strategy)

ระบบใช้ GitHub Actions สำหรับทำ CI/CD เพื่อสร้างและเผยแพร่เวอร์ชันใหม่โดยอัตโนมัติ

### 7.1 ขั้นตอนการปล่อยเวอร์ชันใหม่
1.  **Tagging**: เมื่อมีการ Push git tag ที่ขึ้นต้นด้วย `v*` (เช่น `v0.6.16`) ไปยัง Repository
2.  **Automated Build**: GitHub Actions (`release.yml`) จะทำงานโดยอัตโนมัติ
    - Build แอปพลิเคชันสำหรับ Windows และ macOS
    - สร้าง Installer และ Portable package
3.  **GitHub Release**: ระบบจะสร้าง Release ใหม่ใน GitHub Releases page พร้อมแนบไฟล์ที่ Build เสร็จแล้วให้ดาวน์โหลด

### 7.2 การอัปเดตสำหรับผู้ใช้
- **Automatic Updates**: แอปพลิเคชันมีระบบ `electron-updater` ตรวจสอบเวอร์ชันใหม่จาก GitHub Releases
- **Manual Update**: ผู้ใช้สามารถดาวน์โหลดไฟล์เวอร์ชันล่าสุดจากหน้า Releases มาวางทับได้ (สำหรับ USB Portable version)
